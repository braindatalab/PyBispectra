    ...
    def _sort_indices(self, indices: tuple[tuple[int]] | None) -> None:
        ...
        if indices is None:
            indices = tuple(
                [
                    tuple(
                        np.tile(
                            range(self._n_chans), self._n_chans**2
                        ).tolist()
                    ),
                    tuple(
                        np.repeat(
        ...
        for group_idcs in indices:
            ...
            if any(not isinstance(idx, int) for idx in group_idcs):
                raise TypeError(
                    "Entries for groups in `indices` must be ints."
                )
...
class Bispectrum(_General):
    ...
    def compute(
        ...
        indices : tuple of tuple of int, length of 3 | None (default None)
            Indices of the channels :math:`k`, :math:`m`, and :math:`n`,
            respectively, to compute the bispectrum for. If :obj:`None`, the
            bispectrum for all channel combinations is computed.

        f1s : tuple of int or float, length of 2 | None (default None)
            Start and end lower frequencies to compute the bispectrum for,
        ...
        The bispectrum is computed between all values of :attr:`f1s` and
        :attr:`f2s`. If any value of :attr:`f1s` is higher than :attr:`f2s`, a
...
class Threenorm(_General):
    ...
    def compute(
        ...
        indices : tuple of tuple of int, length of 3 | None (default None)
            Indices of the channels :math:`k`, :math:`m`, and :math:`n`,
            respectively, to compute the threenorm for. If :obj:`None`, the
            threenorm for all channel combinations is computed.

        f1s : tuple of int or float, length of 2 | None (default None)
            Start and end lower frequencies to compute the threenorm for,
            respectively. If :obj:`None`, all frequencies are used.

        f2s : tuple of int or float, length of 2 | None (default None)
            Start and end higher frequencies to compute the threenorm for,
            respectively. If :obj:`None`, all frequencies are used.
        ...
        :math:`\textbf{N}_{kmn}(f_1,f_2)=(<|\textbf{k}(f_1)|^3><|\textbf{m}
        (f_2)|^3><|\textbf{n}(f_2+f_1)|^3>)^{\frac{1}{3}}` ,
        ...
        The threenorm is computed between all values of :attr:`f1s` and
        :attr:`f2s`. If any value of :attr:`f1s` is higher than :attr:`f2s`, a
        :obj:`numpy.nan` value is returned.
